...
1. 首先我们以```github.com/quarkcms/quark-easy```命名模块，执行```go mod init github.com/quarkcms/quark-easy```命令，然后参照 [快速开始](./installation.html#%E5%BF%AB%E9%80%9F%E5%BC%80%E5%A7%8B) 章节，创建好 [main.go](https://github.com/quarkcms/quark-easy/blob/main/main.go) 文件；
2. 根据约定创建如下目录（也可自由命名）：
~~~
www                         WEB部署目录
├─internal                  业务目录
...
└─main.go                   主文件
~~~
3. 打开 models 目录，创建 [post.go](https://github.com/quarkcms/quark-easy/blob/main/internal/models/post.go) 模型文件；
...
```
5. 打开 resources 目录，创建 [post.go](https://github.com/quarkcms/quark-easy/blob/main/internal/admin/resources/post.go) 资源文件；
...
import (
	...
	"github.com/quarkcms/quark-easy/internal/models"
)
...
```
7. 将资源注册到 [providers.go](https://github.com/quarkcms/quark-easy/blob/main/internal/admin/providers.go) 文件里，代码如下：
...
import (
	...
	"github.com/quarkcms/quark-go/pkg/builder"
	adminproviders "github.com/quarkcms/quark-easy/internal/admin"
...
9. 重启服务后，我们打开 ```http://127.0.0.1:3000/admin/#/index?api=/api/admin/post/index``` 路径，你就可以看到文章的页面了；至此一个简单的 CURD 就完成了，完整的项目代码请打开 [Demo](https://github.com/quarkcms/quark-easy) 链接查看
