...
1. 首先我们以```github.com/quarkcms/quark-smart```命名模块，执行```go mod init github.com/quarkcms/quark-smart```命令，然后参照 [快速开始](./installation.html#%E5%BF%AB%E9%80%9F%E5%BC%80%E5%A7%8B) 章节，创建好 [main.go](https://github.com/quarkcloudio/quark-smart/blob/v2/main.go) 文件；
2. 根据约定创建如下目录（也可自由命名）：
~~~
www                         WEB部署目录
├─internal                  业务目录
...
```
5. 打开 resource 目录，创建 [article.go](https://github.com/quarkcloudio/quark-smart/blob/v2/internal/admin/service/resource/article.go) 资源文件；
...
```
7. 将资源注册到 [provider.go](https://github.com/quarkcloudio/quark-smart/blob/v2/internal/admin/service/provider.go) 文件里，代码如下：
